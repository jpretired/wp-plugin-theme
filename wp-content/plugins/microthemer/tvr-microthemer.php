	...
	if (!class_exists('tvr_microthemer_admin')) {
		...
		class tvr_microthemer_admin {
			...
			function ensure_defined_preferences($full_preferences){
				...
				foreach ($full_preferences as $key => $value){
					if ( !isset($this->preferences[$key])){
						...
						/*$this->log(
							esc_html__('Pref set: '.$key, 'microthemer'),
							'<p>Done</p>',
							'notice'
						);*/
					}
				...
				if ($update or $update2){
					...
					/*$this->log(
						esc_html__('We did add extra prefs', 'microthemer'),
						'<p>Done</p>',
						'warning'
					);*/
				} else {
					/*$this->log(
						esc_html__('We did NOT add extra prefs', 'microthemer'),
						'<p>Done</p>',
						'warning'
					);*/
                }
			...
			function updateRevisions($save_data, $user_action = '', $tryCreate = true, $preferences = false) {
				...
				$revs = $wpdb->get_results("select id from $table_name where saved != 1 order by id asc");
				if ($wpdb->num_rows > $max_revisions) {
                    ...
                    $sql = "delete from $table_name where saved != 1 order by id asc limit $excess_rows";
					$wpdb->query($sql);

                    /*$wpdb->query(
					    $wpdb->prepare("delete from $table_name where id = %d and saved != 1", $revs[0]->id)
                    );*/
				}
				return true;
			}

...
<div id="parent_child_options_panel" class="ctc-option-panel<?php echo 'parent_child_options' == $active_tab ? ' ctc-option-panel-active' : ''; ?>">
    <form id="ctc_load_form" method="post" action="">
        ...
        <?php   // theme inputs 
   $this->ctc()->fingerprint_field(); wp_nonce_field( 'ctc_update' ); 
...
?><input type="hidden" name="ctc_analysis" value=""/>
        ...
        <p class="howto indent sep">
          <?php _e( 'Install a new customizable child theme using an installed theme as a parent.', 'child-theme-configurator' ); ?>
                ...
                <?php if ( $this->ctc()->get_theme_count( 'child' ) ): ?>
        ...
        <p class="howto indent sep">
          <?php _e( 'Make a complete copy of an existing Child Theme in a new directory, including any menus, widgets and other Customizer settings. The option to copy the Parent Theme settings is disabled with this action.', 'child-theme-configurator' ); ?>
        </p>
        </label>
        ...
        <?php 
            /**
             * Parent or Child Theme selection options
             */ 
            ?>
        ...
        <div class="ctc-input-row clearfix ctc-themeonly-container" id="input_row_new_theme_option" style="display:none">
            ...
            <div class="ctc-input-cell">
                <?php $this->render_theme_menu( 'parnt', $this->ctc()->get_current_parent(), 'ctc_theme_parnt' ); ?>
        ...
        <div class="ctc-input-row clearfix ctc-themeonly-container" id="input_row_existing_theme_option" style="display:none">
            ...
            <div class="ctc-input-cell">
                <?php $this->render_theme_menu( 'child', $this->ctc()->get_current_child(), 'ctc_theme_child' ); ?>
                <input type="button" class="button button-primary ctc-analyze-theme" value="<?php _e( 'Analyze', 'child-theme-configurator' ); ?>"/>
            </div>
            <div class="ctc-input-cell"><span class="ctc-analyze-howto"><span class="ctc-step ctc-step-number">3</span>
                <strong>
        ...
        <?php _e( 'This is NOT the name of the Child Theme. You can customize the name, description, etc. in step 8, below.', 'child-theme-configurator' ); ?></span>
        ...
        <div class="ctc-input-row clearfix ctc-themeonly-container" id="input_row_stylesheet_handling_container" style="display:none">
          ...
          <p class="howto indent sepstylesheet-container"><strong>Stylesheet File Name</strong><br/>
              <input class="ctc_text ctc-themeonly" id="ctc_sepstylesheet" name="ctc_sepstylesheet" 
                     type="text" value="<?php echo $sepstylesheet; ?>" style="max-width:30rem"
                     placeholder="<?php _e( 'Stylesheet File Name', 'child-theme-configurator' ); ?>" autocomplete="off"/>
                </p>
            </div>
        </div>
...
<p class="howto indent"><?php _e( 'Let the Configurator (try to) resolve any stylesheet issues listed above. This can fix many, but not all, common problems.', 'child-theme-configurator' ); ?></p></label>
             ...
             */ 
            ?>
            <div id="ctc_link_swap_container" style="display:none">
                <div class="ctc-input-cell ctc-clear">
                    <p class="howto">
                    </p>
                </div>
                <div class="ctc-input-cell-wide sep">
                    <strong class="indent">
                        <?php _e( 'Do not reassign urls for these stylesheet handles:', 'child-theme-configurator' ); ?>
                    </strong>
                    <p id="ctc_swaps"></p>
                    <p class="howto indent">
                        <?php _e( 'By default, stylesheet links that point to the child theme are re-registered to point to the parent. This behavior can be disabled for specific stylesheets above.', 'child-theme-configurator' ); ?>
                    </p>
                </div>
            </div>
        ...
        <?php 
            /**
             * Additional stylesheet options - checkboxes populated by javascript
             */ 
            ?>
        <div class="ctc-input-row clearfix ctc-themeonly-container" id="input_row_addl_css_container" style="display:none">
            <div class="ctc-input-cell ctc-clear" id="input_row_addl_css"><span class="ctc-step ctc-step-number">7</span>
                <strong class="shift">
                    <?php _e( 'Select Additional Stylesheets to be used for baseline Styles:', 'child-theme-configurator' ); ?>
                </strong>
                <p class="howto" style="padding-left:48px">
                    <?php _e( 'Stylesheets besides than the primary theme stylesheet file can be loaded into the Configurator editor for customizing. Note that only stylesheets that are queued on the home page will be available for loading here.', 'child-theme-configurator' ); ?>
                </p>
            </div>
            <div class="clearfix ctc-input-cell-wide sep" id="ctc_stylesheet_files" style="display:none">
                <div id="ctc_additional_css_files">
                </div>
            </div>
        </div>
        ...
        <?php 
            /**
             * Customize Child Theme Info
             */ 
            ?>
        <div class="ctc-input-row clearfix ctc-themeonly-container" id="ctc_child_header_parameters" style="display:none">
            <div class="ctc-input-cell"><span class="ctc-step ctc-step-number">8</span>
                <strong>
                    <?php _e( 'Customize the Child Theme Name, Description, Author, Version, etc.:', 'child-theme-configurator' ); ?>
                </strong>
            </div>
            <div class="ctc-input-cell">
                <input type="button" class="ctc-section-toggle button-secondary" value="<?php _e( 'Show/Hide Child Theme Attributes', 'child-theme-configurator' ); ?>" id="ctc_theme_attributes" title="<?php _e( 'Click to toggle form', 'child-theme-configurator' ); ?>"/>
            </div>
        ...
        <div class="ctc-input-row clearfix ctc-themeonly-container" id="ctc_copy_theme_mods" style="display:none">
            <div class="ctc-input-cell">
                <label for="ctc_parent_mods"><span class="ctc-step ctc-step-number">9</span><strong>
        ...
        <div class="ctc-input-row clearfix" id="ctc_configure_submit" style="display:none">
            <div class="ctc-input-cell"><span class="ctc-step ctc-step-number">10</span>
                <strong class="shift">
